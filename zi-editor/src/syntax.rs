...
use tree_sitter::{Node, Query, QueryCapture, QueryCaptures, QueryCursor, Tree};
use zi_core::PointRange;
use zi_text::{AnyText, AnyTextMut, AnyTextSlice, Deltas, TextSlice};

pub(crate) use self::highlight::{HighlightId, HighlightMap, HighlightName, Theme};
use crate::{dirs, FileType};

pub trait Syntax {
    fn new(ft: FileType) -> anyhow::Result<Option<Self>>
    where
        Self: Sized;
...
//     }
//
//     pub fn capture_index_to_name(&self, idx: u32) -> &str {
//         self.highlights_query.capture_names()[idx as usize]
//     }
// }
//
