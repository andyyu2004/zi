...
fn assert_offset_iter_eq<T: Copy + Eq + fmt::Debug + 'static>(
    ...
    let values = values.map(|(offset, &item)| (offset, item)).collect::<Vec<_>>();
...
fn marktree_range_iter() {
    ...
    assert_offset_iter_eq(tree.items(0..0), []);
    assert_offset_iter_eq(tree.items(0..=0), [(0, 0)]);
    assert_offset_iter_eq(tree.items(0..1), [(0, 0)]);
    assert_offset_iter_eq(tree.items(0..2), [(0, 0), (1, 1)]);
    assert_offset_iter_eq(tree.items(1..2), [(1, 1)]);
    assert_offset_iter_eq(tree.items(2..2), []);
    ...
    assert_offset_iter_eq(tree.items(0..3), [(0, 0), (1, 1), (2, 2)]);
    assert_offset_iter_eq(tree.items(0..=2), [(0, 0), (1, 1), (2, 2)]);
    assert_offset_iter_eq(tree.items(20..40), (20..40).map(|i| (i, i as u64)));
    assert_offset_iter_eq(tree.items(80..100), (80..100).map(|i| (i, i as u64)));
    assert_offset_iter_eq(tree.items(80..=100), (80..100).map(|i| (i, i as u64)));
}
...
fn marktree_delete() {
    ...
    tree.delete(0);
    assert_offset_iter_eq(tree.items(..), [(0, 1)]);
...
fn marktree_simple_insert() {
    ...
    tree.insert(1, 1);
    assert_offset_iter_eq(tree.items(..), [(1, 0), (1, 1)]);

    tree.insert(0, 2);
    assert_offset_iter_eq(tree.items(..), [(0, 2), (1, 0), (1, 1)]);
}
...
fn marktree_bulk_insert() {
    ...
    fn check<const N: usize>() {
        ...
        values.iter().for_each(|&(at, i)| {
            tree.insert(at, i);
            assert_eq!(tree.len(), LEN);
            assert_iter_eq(tree.items(..).map(|(a, _)| a), 0..=at);
        });
    }
...
fn marktree_edit_append() {
    let mut tree = MarkTree::<u64, 32>::new(0);
    tree.edit(&deltas![0..0 => "a"]);
    assert_offset_iter_eq(tree.items(..), []);
...
fn marktree_edit_delete() {
    let mut tree = MarkTree::<u64, 32>::new(3);
...
fn marktree_edit() {
    ...
    assert_offset_iter_eq(tree.items(..), [(3, 0)]);
    assert_eq!(tree.len(), 12);

    tree.edit(&deltas![0..1 => ""]);
    assert_eq!(tree.len(), 11);
    assert_offset_iter_eq(tree.items(..), [(2, 0)]);
}
...
fn marktree_smoke() {
    ...
    assert_offset_iter_eq(tree.items(..), [(0, 0), (3, 1)]);

    tree.insert(3, 2);
    assert_offset_iter_eq(tree.items(..), [(0, 0), (3, 1), (3, 2)]);

    tree.insert(2, 4);
    assert_offset_iter_eq(tree.items(..), [(0, 0), (2, 4), (3, 1), (3, 2)]);
    assert_eq!(tree.len(), 10);
}
