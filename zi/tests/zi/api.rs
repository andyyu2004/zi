...
pub async fn new(content: &str) -> zi::Editor {
    new_with_size(content, zi::Size::new(80, 24)).await
}

pub async fn new_with_size(content: &str, view_size: zi::Size) -> zi::Editor {
    ...
    let buf = editor.open_active(tmpfile.path()).expect("failed to open buffer").await.unwrap();
